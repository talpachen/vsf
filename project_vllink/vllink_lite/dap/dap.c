        ...
        uint8_t* response, uint16_t pkt_size)
{
    ...
    do {
        ...
        #if SWO_UART || SWO_MANCHESTER
        else if ((cmd_id >= ID_DAP_SWO_Transport) && (cmd_id <= ID_DAP_SWO_ExtendedStatus)) {
            uint32_t ret = dap_swo_request_handler(param, request + req_ptr, response + resp_ptr, cmd_id, pkt_size - resp_ptr);
            if (ret == 0)
                goto fault;
            else {
                req_ptr += ret & 0xffff;
...
{
    dap_request_t* request = &this_ptr->request[this_ptr->request_head];
    ...
    on_vsf_task_init() { vsf_sem_init(&this_ptr->request_sem, 0); }

    switch (vsf_task_state) {
    case WAIT_FOR_REQ_SEM:
        vsf_task_wait_until(vsf_sem_pend(&this_ptr->request_sem));

        request = &this_ptr->request[this_ptr->request_head];

        this_ptr->response.response_sem = request->response_sem;
        this_ptr->response.response = request->response;
        this_ptr->response.response_param = request->response_param;
        this_ptr->response.response_size = request_handler(this_ptr->dap_param, request->request_buf,
            this_ptr->response.response_buf, request->pkt_size);

        vsf_gint_state_t orig = vsf_disable_interrupt();
        if (++this_ptr->request_head == DAP_PACKET_COUNT)
            this_ptr->request_head = 0;
        this_ptr->request_cnt--;
        vsf_set_interrupt(orig);
        
        vsf_task_state = WAIT_FOR_RESP_SEM;

    case WAIT_FOR_RESP_SEM:
        vsf_task_wait_until(vsf_sem_pend(this_ptr->response.response_sem));

        this_ptr->response.response(this_ptr->response.response_param,
            this_ptr->response.response_buf,
            this_ptr->response.response_size);
...
{
    #if TIMESTAMP_CLOCK
    vsfhal_timestamp_init(TIMESTAMP_CLOCK, PERIPHERAL_TIMESTAMP_PRIORITY);
    #endif
...
vsf_err_t dap_requset_sync(dap_t *dap, uint8_t *request_buf, uint8_t *response_buf, uint16_t pkt_size)
{
    ...
    response_buf = dap_task->response.response_buf;
    return request_handler(param, request_buf, response_buf, min(pkt_size, DAP_PACKET_SIZE));
}
...
void dap_test(dap_t *dap, uint8_t port, uint16_t speed_khz)
{
    ...
    if (param->port == DAP_PORT_SWD) {
        ...
        const uint8_t sreq_ID_DAP_Transfer10[] = {
            0x05, 0x00, 0x07, 0x04, 0x00, 0x00, 0x00, 0x50, 
            0x00, 0x1e, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 
            0x00, 0x00, 0x01, 0x02, 0x00, 0x00, 0xe3, 0x05, 
            0x00, 0xed, 0x00, 0xe0, 0x0f, 0x06
        };
#endif
