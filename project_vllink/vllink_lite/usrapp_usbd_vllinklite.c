typedef struct usbd_vllinklite_nonconst_t {
    struct {
        #ifndef APP_CFG_SERIAL_HEADER_STR_ARRAY_LEN
        uint8_t str_serial[50];
        #else
        uint8_t str_serial[APP_CFG_SERIAL_HEADER_STR_ARRAY_LEN + 2];
        #endif
    } usbd;
...
describe_block_stream(cdcext_ext2usb_stream, 3, APP_CFG_CDCEXT_PKT_SIZE)
describe_block_stream(cdcext_usb2ext_stream, 2, APP_CFG_CDCEXT_PKT_SIZE)
#endif
