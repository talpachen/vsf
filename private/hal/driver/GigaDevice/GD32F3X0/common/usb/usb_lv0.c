...
// TODO: add gd32f3x0_usbh_irq
#define __USB_OTG_IMPLEMENT(__N, __VALUE)                                       \
static const gd32f3x0_usb_const_t __USB_OTG##__N##_const = {                   \
    USB_OTG##__N##_CONFIG                                                       \
};                                                                              \
gd32f3x0_usb_t USB_OTG##__N##_IP = {                                           \
    .param = &__USB_OTG##__N##_const,                                           \
};                                                                              \
ROOT void USB_OTG##__N##_IRQHandler(void)                                       \
{                                                                               \
    if (USB_OTG##__N##_IP.is_host) {                                            \
        gd32f3x0_usbh_irq(&USB_OTG##__N##_IP);                                 \
    } else {                                                                    \
        gd32f3x0_usbd_irq(&USB_OTG##__N##_IP);                                 \
    }                                                                           \
}
...
WEAK(gd32f3x0_usbd_irq)
void gd32f3x0_usbd_irq(gd32f3x0_usb_t *dc) { VSF_ASSERT(false); }
WEAK(gd32f3x0_usbh_irq)
void gd32f3x0_usbh_irq(gd32f3x0_usb_t *dc) { VSF_ASSERT(false); }
