...
#define __GD32F3X0_SWI(__N, __VALUE)                                            \
    ROOT ISR(SWI##__N##_IRQHandler)                                             \
    {                                                                           \
        if (__gd32f3x0_common.swi[__N].handler != NULL) {                       \
            __gd32f3x0_common.swi[__N].handler(__gd32f3x0_common.swi[__N].param);\
        }                                                                       \
    }
...
static const IRQn_Type gd32f3x0_soft_irq[VSF_DEV_SWI_NUM] = {
    VSF_DEV_SWI_LIST
};

typedef struct gd32f3x0_common_t {
    struct {
...
#if __VSF_DEV_SWI_NUM > 0
static gd32f3x0_common_t __gd32f3x0_common;
#endif
...
// SWI
#if __VSF_DEV_SWI_NUM > 0
VSF_MREPEAT(__VSF_DEV_SWI_NUM, __GD32F3X0_SWI, NULL)
                                                ...
                                                void *param)
{
    if (idx < __VSF_DEV_SWI_NUM) {
        if (handler != NULL) {
            __gd32f3x0_common.swi[idx].handler = handler;
            __gd32f3x0_common.swi[idx].param = param;

            NVIC_SetPriority(gd32f3x0_soft_irq[idx], priority);
            NVIC_EnableIRQ(gd32f3x0_soft_irq[idx]);
        } else {
            NVIC_DisableIRQ(gd32f3x0_soft_irq[idx]);
        }
        return VSF_ERR_NONE;
    }
...
{
    if (idx < __VSF_DEV_SWI_NUM) {
        NVIC_SetPendingIRQ(gd32f3x0_soft_irq[idx]);
        return;
    }
    VSF_HAL_ASSERT(false);
}
